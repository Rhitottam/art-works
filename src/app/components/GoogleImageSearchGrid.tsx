...
export default function GoogleImageSearchGrid({ results, grid }: Props) {
  ...
  return (
    ...
    >
      {results.map((result: Item) => {
        return (
          <ImageItem result={result} grid={grid} sum={sum} key={result.link} />
        );
      })}
    </div>
  );
  ...
  return !colSpan ? (
    ...
    >
      <div
        ...
        ref={containerRef}
        style={{
          backgroundImage: isExpanded ? "" : `url(${result?.link})`,
      ...
      >
        ...
        >
          <div className="space-x-1 text-xl font-bold text-gray-500">
            <a href={result.image.contextLink}>{result.image.contextLink}</a>
          </div>
